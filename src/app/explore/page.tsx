...
function ExploreInner() {
  ...
  const ids = useMemo(() => {
    ...
    if (q.trim()) {
      ...
      list = list.filter((a) => {
        ...
        return [a.title, a.description, a.category, a.brandName, brand?.name, brand?.slug].some((v) => (v || "").toLowerCase().includes(needle));
      });
    }
  ...
  }, [ads, brands, q, category, sort]);
  return (
    <div>
      <h1 className="text-2xl font-semibold">Explore</h1>
      <p className="mt-1 text-sm text-neutral-500">Live catalog from CREATE — search, filter, sort.</p>
      <div className="mt-6 flex flex-col gap-3">
        <input value={q} onChange={(e) => { setQ(e.target.value); setParam("q", e.target.value); }} placeholder="Search titles, brands, categories" className="w-full rounded-full border border-neutral-200 bg-white px-4 py-2.5 text-sm outline-none" />
        <div className="flex flex-wrap gap-2">{(["All", ...CATEGORIES] as const).map((c) => (
        ...
        <div className="flex flex-wrap gap-2">{([["latest", "Latest"], ["trending", "Trending"], ["liked", "Most liked"], ["viewed", "Most viewed"]] as const).map(([key, label]) => (
      ...
      <div className="mt-8">
        {!loaded ? <p className="text-sm text-neutral-500">Loading…</p> : ids.length === 0 ? <p className="rounded-3xl border border-dashed p-10 text-center text-sm text-neutral-500">There are no ads</p> : <AdGrid ids={ids} />}
      </div>
    </div>
  );
}
