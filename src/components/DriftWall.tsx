...
type Item = { image: string; title?: string; href?: string };
type Props = {
  items: Item[];
  columns?: number;
  tileWidth?: number;
...
export default function DriftWall({
  ...
  const renderTile = (item: Item, id: string, colIndex: number) => {
    const inner = (
    ...
    if (item.href) return <a key={id} href={item.href} {...common}>{inner}</a>;
    return <div key={id} tabIndex={0} role="button" aria-label={item.title ?? "tile"} {...common}>{inner}</div>;
  };
