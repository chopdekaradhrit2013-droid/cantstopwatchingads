  ...
  return (
    <article className="flex items-center gap-3 rounded-2xl border border-neutral-200 bg-white p-3">
      ...
      <div className="min-w-0 flex-1">
        <Link href={`/brands/${brand.slug}`} className="font-medium">{brand.name}</Link>
        <p className="truncate text-xs text-neutral-500">{formatCount(brand.followers)} followers</p>
      </div>
      <button type="button" onClick={() => guard("follow this brand") && toggleFollow(brand.id)} className={`rounded-full px-3 py-1.5 text-xs ${isFollowed(brand.id) ? "border border-neutral-200" : "bg-neutral-900 text-white"}`}>
