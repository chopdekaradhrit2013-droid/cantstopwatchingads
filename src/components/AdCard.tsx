      ...
      <div className="p-3">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex shrink-0 gap-1">
            <button type="button" onClick={() => guard("like this ad") && toggleLike(ad.id)} className={`rounded-full border px-2.5 py-1 text-xs ${isLiked(ad.id) ? "border-neutral-900 bg-neutral-900 text-white" : "border-neutral-200"}`}>{isLiked(ad.id) ? "Liked" : "Like"}</button>
            <button type="button" onClick={() => guard("save this ad") && toggleSave(ad.id)} className={`rounded-full border px-2.5 py-1 text-xs ${isSaved(ad.id) ? "border-neutral-900 bg-neutral-900 text-white" : "border-neutral-200"}`}>{isSaved(ad.id) ? "Saved" : "Save"}</button>
          </div>
        </div>
