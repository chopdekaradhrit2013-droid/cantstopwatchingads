...
export function readLocalBoard(): AdminBoard {
  ...
  try {
    return { ...emptyBoard(), ...JSON.parse(localStorage.getItem(BOARD_KEY) || "{}") };
  } catch {
    return emptyBoard();
  }
}

export function writeLocalBoard(board: AdminBoard) {
  localStorage.setItem(BOARD_KEY, JSON.stringify(board));
}

export function activeAnnouncements(board: AdminBoard) {
  const now = Date.now();
  return board.announcements.filter((a) => new Date(a.until).getTime() > now);
}
...
export async function pullBoard(): Promise<AdminBoard> {
  ...
  try {
    ...
    const row = (brands as { id: string; description?: string }[]).find((b) => b.id === BOARD_ID);
    if (row?.description) {
      const parsed = { ...emptyBoard(), ...JSON.parse(row.description) } as AdminBoard;
      writeLocalBoard(parsed);
      return parsed;
    }
  } catch {}
  return local;
...
export async function pushBoard(board: AdminBoard) {
  ...
  try {
    await upsertBrand({
      id: BOARD_ID,
      name: "CSWA Admin Board",
      handle: "cswaadmin",
      description: JSON.stringify(board),
      website: "",
      industry: "Other",
      logo: "",
      contact_email: "chopdekaradhrit2013@gmail.com",
      followers: 0,
    });
  } catch {}
}
